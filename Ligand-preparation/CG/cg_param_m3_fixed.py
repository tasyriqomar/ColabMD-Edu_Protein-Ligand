...
def get_diffs(alogps, ring_size, frag_size, category, size):
    key = ring_size - frag_size
    if key not in delta_Gs:
        print(f"[WARN] Invalid delta_Gs key: ring_size={ring_size}, frag_size={frag_size}, key={key}. Using delta_Gs[0] as fallback.")
        key = 0  # fallback to default
    try:
        diffs = np.abs(np.array(delta_Gs[key][category][size]) - alogps)
    except KeyError as e:
        raise KeyError(f"Invalid lookup in delta_Gs with key={key}, category={category}, size={size}.") from e
    return diffs
...
def get_alogps(bead_smi):
    # Gets ALOGPS value from server. If this fails, fallback to Wildman-Crippen
    try:
        alogps = requests.get('http://vcclab.org/web/alogps/calc?SMILES=' + bead_smi).text
    except:
        mol = Chem.MolFromSmiles(bead_smi)
        if mol is None:
            print(f"[ERROR] Cannot parse SMILES for Crippen descriptor: {bead_smi}")
            return 0.0  # fallback default
        logK = rdMolDescriptors.CalcCrippenDescriptors(mol)[0]
        print(bead_smi, 'Data from Wildmann-Crippen (fallback)')
        return logK * 5.74
